...
app.post("/api/sensor", async (req, res) => {
  ...
  try {
    ...
    // PERBAIKAN: Gunakan typeof untuk mengecek apakah field ada (bukan undefined)
    if (
      typeof h_kanan === "undefined" ||
      typeof h_kiri === "undefined" ||
      typeof q_kanan === "undefined" ||
      typeof q_kiri === "undefined"
    ) {
      return res
        .status(400)
        .json({ 
    ...
    console.log(`[${formatWIB(wibNow)}] 📊 Sensor data:`, sensorData);
    ...
    try {
      ...
      console.log(
        `[${formatWIB(wibNow)}] ✅ ML Prediction received:`,
        mlPrediction
      );

      if (
        mlPrediction.status === "BANJIR" ||
        mlPrediction.status === "BAHAYA" ||
        mlPrediction.prediction === 1
      ) {
        shouldSendAlert = true;
        console.log(
          `[${formatWIB(wibNow)}] 🚨 Flood condition detected! Will send alert.`
        );
      }
    } catch (mlError) {
      ...
      if (sensorData.h_kanan > 150 || sensorData.h_kiri > 150) {
        ...
        console.log(
          `[${formatWIB(
            wibNow
          )}] ⚠️ Using manual threshold - Flood condition detected!`
        );
      }
    }
    ...
    if (shouldSendAlert && mlPrediction) {
      ...
      try {
        ...
        if (telegramResult) {
          console.log(
            `[${formatWIB(wibNow)}] ✅ Telegram alert sent successfully`
          );
        } else {
          console.warn(
            `[${formatWIB(wibNow)}] ⚠️ Telegram alert returned false`
          );
        }
      } catch (telegramError) {
        console.error(
          `[${formatWIB(wibNow)}] ❌ Failed to send Telegram alert:`,
  ...
  try {
    ...
    if (latestReading) {
      try {
        ...
        console.warn(
          "⚠️ ML API unavailable, using threshold:",
          mlError.message
        );
  ...
  try {
    ...
    try {
      const { data: latestReading, error: latestError } = await supabase
        ...
        .order("created_at", { ascending: false })
        .limit(1);
      if (!latestError && latestReading?.length > 0)
        latestData = latestReading[0];
    } catch (e) {
...
(async () => {
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      console.warn(
        "⚠️ Telegram test failed (might be normal if no credentials):",
        error.message
      );
    }
  }, 2000);
